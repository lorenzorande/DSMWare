...
def Parsed(httpString) :
	...
	toparse = parsed['Body']
	toparse += '&'
	newparsed=dict(p_glob.findall(toparse))
...
def ParseToken(request_token) :
	p_glob = re.compile("\s*([^&=]*)=([^&=]*)&")
	request_token += '&'
	newparsed=dict(p_glob.findall(request_token))
	return newparsed
...
def ClientHandler(clientHttpMessage) :
	...
	host = parser.parser(clientHttpMessage)['host']
	path = parser.parser(clientHttpMessage)['path']
	...
	consumer_key=Parsed(clientHttpMessage)['oauth_consumer_key'] # on identifie le client
	...
	# il s agit d un message d authentification
		...
		# client en est a etape 1 de l authentification (il a fait login)
			...
			request_token = sess.obtain_request_token_proxy(Parsed(clientHttpMessage)['oauth_timestamp'], Parsed(clientHttpMessage)['oauth_nonce'], Parsed(clientHttpMessage)['oauth_version'])
		...
		if path.split("/")[3] == "access_token" :
			...
			new_request_token=sess.obtain_access_token_proxy(Parsed(clientHttpMessage)['oauth_timestamp'], Parsed(clientHttpMessage)['oauth_nonce'], Parsed(clientHttpMessage)['oauth_version'],request_token)
